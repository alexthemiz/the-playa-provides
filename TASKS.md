...
- [ ] **Verify profile 2026 section on prod** — Burning Man 2026 status now sits above the Playa History heading in both published and edit views (excluded from the year-chip list below, which ends at 2025). Published: no box, "Burning Man 2026:" label + status badge + camp/Open Camping/TBD all inline on one row. Edit: camp name field above the Yes/Maybe/No/Open Camping/TBD row; TBD is a new checkbox (mutually exclusive with Open Camping and typing a camp name) that maps to the same "no camp set" row as before — no schema change.

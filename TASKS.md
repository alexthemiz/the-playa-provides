...
- [ ] **Verify camp members table Wish List — confirmed working on device, now needs a check of the 3-row + header-bg follow-up** — the `grid-auto-flow:column` redesign (exactly-N-rows by construction, grows wider instead of clipping, flat px grid columns to prevent per-row drift) was confirmed working by Alex on his real device after two earlier broken attempts (clipping via `max-height`+`overflow:hidden`; a `minWidth` scroll-trigger that worked in the browser-pane Chromium but not his real browser). Follow-up shipped 2026-07-22: row cap raised 2→3 (`gridTemplateRows: repeat(3, auto)`), and the header row's tan background now spans the full table width — previously each row (including the header) sized itself independently via `width: max-content`, so the header's shorter content (just column labels) left its background narrower than data rows with long wish lists; fixed by wrapping the header + all member rows in one shared `width: max-content` container and changing each row to `width: 100%` of that shared box. Verified via a standalone repro matching the real CSS exactly (header/row widths match exactly at both 1000px and 375px, scroll only engages when content truly overflows, wish-list cells hold at exactly 3 rows regardless of tag count) but **not yet checked on Alex's actual device** — same class of fix that broke silently last time between the browser pane and his real browser.

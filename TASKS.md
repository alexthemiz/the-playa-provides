...
- [ ] **Verify lend/transfer email lookup fix on prod** — the recipient lookup was searching `profiles.contact_email` (optional, set only via Settings) instead of the login `profiles.email` (always set at signup), so 63% of accounts (76/120) — anyone who never filled in a contact email — returned "No account found" when someone tried to lend/transfer to them by email. Now matches `email OR contact_email` in both LendModal and TransferModal. Confirm you can now find a real user (e.g. the friend who prompted this) by their signup email.

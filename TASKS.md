...
- [ ] **Verify camp members table Wish List — simplified redesign, needs a real check on Alex's device** — after five increasingly complex attempts at "cap every row at exactly ≤3 lines, never clip, use available space" (CSS grid column-locking → JS width measurement → a `1fr` sizing bug → a webfont-load race), Alex stepped back and asked what a simple, professional design would actually look like. Answer, matching GitHub topics / Notion multi-select / Trello labels: **no row cap at all**. Chips flex-wrap naturally within a `minmax(300px, 1fr)` column; a member with a big wish list just gets a taller row. `WishListCell` dropped to ~10 lines with zero JS measurement, refs, or listeners. Two small *static* CSS bugs surfaced and got fixed while simplifying (both single properties, no measurement): `box-sizing: border-box` (width:100% + padding was otherwise ~30px too wide, a permanent unwanted scrollbar) and a static `min-width` on the row/header (sum of the fixed columns + gaps + padding + the 300px wish floor) so a narrow viewport can't shrink the row below what its fixed columns need — without it, the row's content overflowed its own box while the header's tan background stayed clamped, the exact background-mismatch bug from earlier this session. Verified via a plain static-HTML repro (no JS at all) at 1900px and 375px: rows render at natural height, header matches every row exactly in both cases, scroll only appears when the fixed columns genuinely don't fit, nothing clips — but **not yet checked on Alex's actual device**.

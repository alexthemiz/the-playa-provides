...
- [ ] **Verify profile 2026 section on prod** — order is now Playa History heading → hint → 2026 status row → year chips (2025 and earlier; 2026 excluded from the chip list). 2026 row has no box, reads exactly "2026: <status badge> Camp: <camp name / Open Camping / TBD / N/A>" (N/A for Not Attending). Edit mode: camp name field above the Yes/Maybe/No/Open Camping/TBD row; TBD is a checkbox (mutually exclusive with Open Camping and typing a camp name) that maps to the same "no camp set" row as leaving it blank — no schema change.

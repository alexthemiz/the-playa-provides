...
- [x] **Camp members table Wish List — simplified, confirmed working on device 2026-07-22.** After five increasingly complex attempts at "cap every row at exactly ≤3 lines, never clip, use available space" (CSS grid column-locking → JS width measurement → a `1fr` sizing bug → a webfont-load race), stepped back to what a simple design actually looks like: **no row cap at all**, matching GitHub topics / Notion multi-select / Trello labels. Chips flex-wrap naturally within a `minmax(300px, 1fr)` column; a member with a big wish list just gets a taller row. `WishListCell` is ~10 lines with zero JS. Two static CSS bugs fixed along the way: `box-sizing: border-box` (width:100% + padding was ~30px too wide, a permanent unwanted scrollbar) and a static `min-width` on the row/header so a narrow viewport can't shrink the row below what its fixed columns need (otherwise the header's tan background fell short once scrolled).
- [ ] **Verify camp members table row alignment fix on prod** — on mobile (horizontally scrolled to the left columns, Wish List off-screen), a row's height is driven by its tallest cell, so a member with a long wish list got a visibly taller row — and with the row's `alignItems: center`, that member's name/avatar floated in the middle of the tall row, reading as a random gap before/after them (reported live via screenshot, Ander/Alex/Jules). Fixed 2026-07-22 by switching to `alignItems: start` — every cell now begins at the row's top edge, so extra height from a long wish list trails below the name instead of surrounding it. Verified via repro: name/avatar top position is exactly `rowTop + 10px` (the padding) in every row regardless of that row's overall height. Not yet checked on Alex's device.

...
- [ ] **Verify camp members table Wish List cap + scroll fix on prod** — Wish List cell caps to exactly 2 rows of tags (`max-height: 56px; overflow: hidden`) instead of growing a member's whole row taller. First pass relied on the grid row implicitly overflowing its `overflow-x:auto` parent to trigger scroll, which worked in the browser pane's Chromium but Alex reported it didn't on his end (table just clipped, no scroll) — likely a Safari/WebKit intrinsic-sizing difference. Fixed by giving the row an explicit `minWidth` (760px / 920px edit mode) matching its own column-template minimums, so the row's box reliably resolves to that width in any engine instead of depending on overflow-propagation behavior that varies by browser. No "+N more" indicator; overflow tags are silently clipped. **This one needs an actual narrowed-window check on Alex's real browser/device**, not just prod load — that's the failure mode being fixed.

...
- [ ] **Verify camp members table Wish List cap on prod** — the Wish List cell now caps to exactly 2 rows of tags (`max-height: 56px; overflow: hidden`) instead of growing a member's whole row taller for a long wish list. The column's existing 300px min-width + the table's horizontal scroll (both pre-existing) keep that 2-row cap holding at any window width — verified at 375px that the table scrolls instead of squeezing the column narrower. No "+N more" indicator; overflow tags are silently clipped.

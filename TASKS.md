...
- [ ] **Verify profile 2026 section on prod** — order is Playa History heading → hint → 2026 status row → year chips (2025 and earlier; 2026 excluded from the chip list). Published row reads exactly "2026: <status badge> Camp: <camp name / Open Camping / TBD / N/A>" (colored per case: camp=tan+teal link, N/A=red, TBD=yellow, Open Camping=teal). Edit mode is two rows — "Attending In 2026?" + Yes/Maybe/No, then "Camp:" + camp field/Open Camping/TBD checkboxes — the Camp row no longer disappears; picking No disables it to a grayed "N/A" instead. "Previous Years:" replaces the old instructional sentence above the year-entry rows.

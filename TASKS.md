...
- [x] **Camp members table Wish List — simplified, confirmed working on device 2026-07-22.** After five increasingly complex attempts at "cap every row at exactly ≤3 lines, never clip, use available space" (CSS grid column-locking → JS width measurement → a `1fr` sizing bug → a webfont-load race), stepped back to what a simple design actually looks like: **no row cap at all**, matching GitHub topics / Notion multi-select / Trello labels. Chips flex-wrap naturally within a `minmax(500px, 1fr)` column (widened from a 300px floor same day, so fewer lines needed before wrapping); a member with a big wish list just gets a taller row. `WishListCell` is ~10 lines with zero JS. Two static CSS bugs fixed along the way: `box-sizing: border-box` (width:100% + padding was ~30px too wide, a permanent unwanted scrollbar) and a static `min-width` on the row/header so a narrow viewport can't shrink the row below what its fixed columns need (otherwise the header's tan background fell short once scrolled). Row content is top-aligned (`alignItems: start`), not centered — a tall row (long wish list) trails extra space below the name instead of floating it in the middle. Alex flagged that big row heights on mobile still aren't quite what he wants but is leaving it as-is for now; revisit if it keeps bugging him.

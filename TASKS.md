...
- [ ] **Verify camp members table Wish List — 3rd attempt, needs a real check on Alex's device** — went through two broken iterations before landing on the current approach, logged here so we don't repeat them. (1) `max-height`+`overflow:hidden` clipped excess tags instead of growing the table to show them, and the guessed pixel height cut chips off mid-height on the real page. (2) Adding `minWidth` to force horizontal scroll worked in this browser pane's Chromium but not in Alex's real browser (desktop: no scroll at all, content just clipped) — an unreliable auto-width-overflow-propagation issue. Current fix: wish-list cell is `grid-auto-flow:column` + `grid-template-rows: repeat(2, auto)` (always exactly 2 rows, grows wider — more columns — to fit every tag, no clipping, no overflow:hidden); `memberGridStyle` columns changed from ranged (`minmax(140px,180px)`) to flat px (rows are independent grids, so a range could silently resolve differently per row); row width changed from default `auto` to explicit `width: max-content` so the row's box deterministically equals its real content size instead of relying on auto-width overflow propagation. Verified in-browser: exact 2-row cap with zero clipping, Name/Location columns stay pixel-aligned across rows of differing wish-list length, scroll only appears when actually needed (a normal list fits at 1000px with no scroll; a 12-tag list at the same width correctly triggers it). **Still needs a check on Alex's actual browser/device** — that's the exact failure mode this is fixing and the browser pane can't reproduce it.

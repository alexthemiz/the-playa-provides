...
**Requires a `middleware.ts` change.** `middleware.ts` uses an opt-in whitelist (`isPublicRoute`) — any path not explicitly listed redirects a logged-out visitor to `/login` before the page renders at all. `/loan-invite` isn't on that list by default, which would silently break the pre-auth preview entirely (the exact thing the point above depends on). This requires adding one line to that whitelist, the same pattern already used for `/find-items` etc. `middleware.ts` is normally do-not-touch-without-discussion in this project; this specific one-line addition was surfaced to and explicitly authorized by Alex during the implementation plan's final review pass, not something to treat as blanket permission for other changes to this file.
...
The claiming user needs no direct table grants on `informal_loans` — the RPC (running as the definer) does all the sensitive reads/writes, matching how `confirm_transfer_receipt` already handles the equivalent trust boundary for real transfers. `app/auth/callback/route.ts` is untouched — claiming happens as a distinct step on the dedicated claim page after a normal login/signup, not inside the auth callback itself. `middleware.ts` *does* need one line (see "Requires a middleware.ts change" above) so the page is even reachable while logged out in the first place; both files are normally flagged in this project as do-not-touch-without-discussion, and the middleware line was specifically surfaced and authorized rather than assumed. Also note: `/login` and `/signup` don't currently support a caller-supplied redirect destination (confirmed by reading both — they hardcode their post-auth landing page), so the claim page doesn't try to auto-return the user after auth; it just tells them to come back to the link, which works with zero changes to either page since the claim page already checks session state on load.

...
"My Camps" is added to the header nav in the same place and the same way as My Inventory / My Profile / Settings: it simply doesn't render for a logged-out visitor. `/camps` (the aggregator) is not added to `middleware.ts`'s public-route whitelist, so it inherits the same default-gated behavior those other pages already get — a logged-out visitor who somehow hits the URL directly gets redirected to `/login`, same as today.

**Update, made later in the same build:** individual camp pages (`/camps/[slug]`) *were* whitelisted for guests, once the on-playa-resources work (below) added a "View camp page →" link on the public `/resources` page — that link is pointless if clicking it just bounces a logged-out visitor to `/login`. This means a logged-out visitor can now see a camp's identity (banner, description) *and* its member roster (names, avatars, wish lists) — the same as what a logged-in non-member already sees today, just now without needing an account. Gear items stay gated behind the existing `isMember` check regardless of login state. This was an explicit, disclosed tradeoff (not an oversight) — approved directly rather than assumed. The bare `/camps` aggregator remains fully gated; only the individual camp pages changed.
